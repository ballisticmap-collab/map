// app.js（全文）
// 通常：ローカルだけ（firebase不要）
// 共有：ボタン押したら firebase.js を動的importして同期
  ...
  el.addEventListener("pointermove", (e) => {
    ...
    if (pinData?.type === "g_smoke" && pointers.has(e.pointerId)) {
      ...
      if (pointers.size === 2 && pinchStartDist) {
        ...
        el.style.setProperty("--pin-size", newSize + "px");
      }
    }
...
shareBtn.addEventListener("click", async () => {
  ...
  try{
    ...
    shareEnabled = false;
    fb = null;
    setStatus("共有失敗（firebase.js/設定/配置）", "error");
...
function initSizeSlider(){
  ...
  slider.addEventListener("input", (e) => {
    selectedPinSize = parseInt(e.target.value);
    sizeValue.textContent = selectedPinSize + "px";
  });
}
